FitCurves <- R6::R6Class(
    ...
    private = list(
        ...
        #' @return None.
        fit_gmm = function(dat, ind, C.value=1, sigma3=2, nr.of.curves){
            ...
            if(nr.of.curves == 1){
                ...
                integral.curve.one <- tryCatch({
                    integrate(
                        ...
                        upper = x[length(x)],
                        C = summary.fit.params["C1", 1],
                        SD = summary.fit.params["sigma1", 1])$value
                    }, error = function(e){
                    integrate(
                        curve.one, 
                        lower = x[1], 
                ...
                integral.curve.two <- tryCatch({
                    integrate(
                        ...
                        upper = x[length(x)],
                        C = summary.fit.params["C2", 1],
                        SD = summary.fit.params["sigma2", 1])$value
                    }, error = function(e){
                    integrate(
                        curve.two, 
                        lower = x[1], 
                ...
                if(nr.of.curves == 2){
                    ...
                    integral.all.curves <- tryCatch({
                        integrate(
                            ...
                            SD2 = summary.fit.params["sigma2", 1])$value
                    },error = function(e){
                        integrate(
                            all.curves, 
                            lower = x[1], 
                    ...
                    integral.curve.three <- tryCatch({
                        integrate(
                            ...
                            upper = x[length(x)],
                            C = summary.fit.params["C3", 1],
                            SD = summary.fit.params["sigma3", 1])$value
                    }, error = function(e){
                        integrate(
                            curve.three, 
                            lower = x[1], 
                    ...
                    integral.all.curves <- tryCatch({
                        integrate(
                            ...
                            SD1 = summary.fit.params["sigma1", 1],
                            SD2 = summary.fit.params["sigma2", 1],
                            SD3 = summary.fit.params["sigma3", 1])$value
                    }, error = function(e){
                        integrate(
                            all.curves, 
                            lower = x[1], 
        ...
        make_plot = function(dat, k, curve.vals, nr.of.curves){
            ...
            if(any(CI.lst[2] > 270 | CI.lst[1] < -270)){
                return(2)
            }

            if(nr.of.curves > 1){
                if(any(1.96*st.devs[[2]] > 270 | -1.96*st.devs[[2]] < -270)){
                    return(2)
                }
            ...
            if(nr.of.curves == 3){
                if(any(1.96*st.devs[[3]] > 270 | -1.96*st.devs[[3]] < -270)){
                    return(2)
                }
        ...
        get_rmsd_tracts = function(kmer){
            all.files <- list.files(
                path = "../data",
                pattern = paste0(ifelse(private$auto_fit, "key", "new"), 
                                 "_stats_kmer_", kmer, ".*"), 
                recursive = TRUE,
                full.names = TRUE
            )
        ...
        #' @description
        #' Save the number of curves fitted for each k-meric RMSD value.
        #' @param kmer Numeric vector of the k-mer size.
        #' @return None.
        save_curve_counts = function(kmer){
            all.files <- list.files(
                path = "../data",
            ...
            Clustering <- function(dat, log_scale){
                ...
                ggsave(
                    plot = dat.plot,
                    filename = paste0("../figures/kmer_", kmer, 
