...
################################################################################
getKmerHybridisationGs <- function(k = 6, plot = FALSE){
  ...
  for(i in 1:length(query.permuts)){
    ...
    triads <- oligonucleotideFrequency(x=DNAStringSet(kmer), width=3, step=1,
  ...
  print(paste0("getKmerHybridisationGs: returning Gfree table for all ",
               k,"-mers."), quote=FALSE)
  return(
    data.table(kmers=query.permuts, Gfrees=Gfrees)
  )
}
################################################################################