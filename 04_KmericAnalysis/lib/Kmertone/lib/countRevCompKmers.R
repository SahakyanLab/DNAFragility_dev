countRevCompKmers <- function(kmer.table, update.count=TRUE) {
  
  # Smartly count the reverse complement sequence from it's contrast strand.
  # Support kmer.table with splitted k-mer columns: kmer_part1, kmer_part2
  #
  # kmer.table     <string>   A variable name pointing to data.table object.
  #                           The data.table must contains three named-columns:
  #                           kmer, pos_strand, and neg_strand
  ...
  if (length(rc.count[is.na(rc.count)]) > 0) {
    # Because all k-mer species should already exist as a result of
    # initKmerTable.R!
    stop("Something is wrong!")
    #rc.count[is.na(rc.count), rc.count := 0]
  }
